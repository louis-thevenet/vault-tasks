...
mod tests {
    ...
    use crate::{
        action::Action,
        components::{explorer_tab::ExplorerTab, Component},
        config::Config,
    };

    #[test]
    fn test_render_explorer_tab_component() {
        let mut explorer_tab = ExplorerTab::new();
        let (tx, _rx) = unbounded_channel();

        explorer_tab.register_action_handler(tx).unwrap();
        explorer_tab
        ...
        explorer_tab.update(None, Action::FocusExplorer).unwrap();
        terminal
            .draw(|frame| explorer_tab.draw(frame, frame.area()).unwrap())
            .unwrap();
        insta::with_settings!({
            info => &"Explorer is focused".to_owned(), // the template context
        }, {
                assert_snapshot!(terminal.backend());
        });

        explorer_tab.update(None, Action::FocusFilter).unwrap();
        terminal
            .draw(|frame| explorer_tab.draw(frame, frame.area()).unwrap())
            .unwrap();
        insta::with_settings!({
            info => &"Explorer is unfocused".to_owned(), // the template context
        }, {
                assert_snapshot!(terminal.backend());
        });
    }
    #[test]
    fn test_move_explorer_tab_component() {
        let mut explorer_tab = ExplorerTab::new();
        let (tx, _rx) = unbounded_channel();
        ...
        explorer_tab.is_focused = true;
        let mut terminal = Terminal::new(TestBackend::new(160, 50)).unwrap();

        explorer_tab.update(None, Action::Down).unwrap();
        terminal
            .draw(|frame| explorer_tab.draw(frame, frame.area()).unwrap())
            .unwrap();
        assert_snapshot!(terminal.backend());

        explorer_tab.update(None, Action::Right).unwrap();
        terminal
            .draw(|frame| explorer_tab.draw(frame, frame.area()).unwrap())
            .unwrap();
        assert_snapshot!(terminal.backend());
