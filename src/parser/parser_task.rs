...
use super::{
    parser_priorities::parse_priority, parser_state::parse_task_state, parser_tags::parse_tag,
    token::Token,
};
...
pub fn parse_task<'a>(
    ...
    let task_state = match parse_task_state(input).map_err(|e| anyhow::format_err!("{e}"))? {
        Token::State(state) => state,
        _ => bail!("Line doesn't represent a task: {input}"),
    };

    let mut token_parser = |input: &mut &str| parse_token(input, config);

    let tokens = input.split_whitespace().map(|token| {
        token_parser
            .parse(token)
...
Description"#
    ...
    fn test_parse_task_no_description() {
        ...
        let config = Config::default();
        let res = parse_task(&mut input, None, &config);
        assert!(res.is_ok());
        let res = res.unwrap();
        let year = chrono::Local::now().year();
        let expected = Task {
